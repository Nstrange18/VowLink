    ...
    <section className="flex min-h-screen items-center justify-center bg-[#070A13] bg-[url('/hero-bg2.png')] bg-cover bg-top bg-no-repeat px-6">
      <div className="w-full max-w-sm rounded-[24px] border border-[#D8B76A]/40 bg-[#070A13]/85 px-8 py-12 shadow-2xl backdrop-blur-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              <input id="login-password" type={showPw ? 'text' : 'password'} placeholder="••••••••"
                {...register('password')}
        ...
        <div className="mt-5 text-center">
          <Link to="/admin/forgot-password" className="text-xs text-white/40 hover:text-[#D8B76A] transition">Forgot password?</Link>
        </div>
        <p className="mt-4 text-center text-sm text-white/40">
          New couple? <Link to="/signup" className="text-[#D8B76A] hover:underline">Create your account</Link>
        </p>
      </div>
    </section>
  )
